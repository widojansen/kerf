defmodule Kerf.Monitor.LLMTelemetryTest do
  ...
  @moduledoc """
  Verifies that LLM providers emit [:kerf, :llm, :call, :stop] and
  [:kerf, :llm, :call, :exception] telemetry events.
  ...
  describe "Instrumentation.emit_call_stop/4" do
    test "emits [:kerf, :llm, :call, :stop] with model, duration, tokens" do
      attach_handler([:kerf, :llm, :call, :stop])
      ...
      assert_receive {:telemetry, [:kerf, :llm, :call, :stop],
  ...
  describe "Instrumentation.emit_call_error/4" do
    test "emits [:kerf, :llm, :call, :stop] with error status" do
      attach_handler([:kerf, :llm, :call, :stop])
