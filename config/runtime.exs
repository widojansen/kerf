...
if api_key = System.get_env("ANTHROPIC_API_KEY") do
  config :exclaw, Kerf.LLM.Provider, api_key: api_key
end
...
if model = System.get_env("EXCLAW_DEFAULT_MODEL") do
  config :exclaw, Kerf.Channels.CLI, model: model
  config :exclaw, Kerf.Channels.Telegram, model: model
  config :exclaw, Kerf.Scheduler, model: model
end
...
if secret = System.get_env("SECRET_KEY_BASE") do
  config :exclaw, Kerf.Dashboard.Endpoint, secret_key_base: secret
...
if config_env() == :prod do
  if database_url = System.get_env("DATABASE_URL") do
    ...
    if db_overrides != [] do
      config :exclaw, Kerf.Repo, db_overrides
    end
  end
end
...
if dashboard_ip = System.get_env("DASHBOARD_IP") do
  ...
  config :exclaw, Kerf.Dashboard.Endpoint, http: [ip: parsed_ip, port: dashboard_port]
end

# SearXNG URL override.
if searxng_url = System.get_env("SEARXNG_URL") do
  config :exclaw, Kerf.Tools.WebSearch, searxng_url: searxng_url
end

# Data directory overrides (for Docker volume mounts).
if data_dir = System.get_env("EXCLAW_DATA_DIR") do
  config :exclaw, Kerf.Memory.Store, data_dir: data_dir
end

if workspaces_dir = System.get_env("EXCLAW_WORKSPACES_DIR") do
  config :exclaw, Kerf.Container.Manager, workspaces_dir: workspaces_dir
end

if fallback_dir = System.get_env("EXCLAW_TELEMETRY_DIR") do
  config :exclaw, Kerf.Telemetry.Logger, fallback_dir: fallback_dir
end
...
if kb_embedding_url do
  config :exclaw, Kerf.KnowledgeBase.Embedder,
...
if System.get_env("EMAIL_TRIAGE_ENABLED") == "true" do
  config :exclaw, Kerf.Ingestors.Email.EmailIngestor,
    enabled: true,
  ...
  config :exclaw, Kerf.Agents.EmailTriage,
    enabled: true,
...
if System.get_env("PHX_SERVER") do
  config :exclaw, Kerf.Dashboard.Endpoint, server: true
end
